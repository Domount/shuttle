...
test("createStore respects SHUTTLE_DATA_DIR env", async () => {
  const dataDir = await mkdtemp(path.join(tmpdir(), "shuttle-store-env-"));
  const memoryDir = await mkdtemp(path.join(tmpdir(), "shuttle-store-mem-"));
  const prev = process.env.SHUTTLE_DATA_DIR;
  process.env.SHUTTLE_DATA_DIR = dataDir;
  try {
    ...
    if (prev === undefined) delete process.env.SHUTTLE_DATA_DIR;
    else process.env.SHUTTLE_DATA_DIR = prev;
  }
});
